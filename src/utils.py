    ...
    if string_date is not None:
        try:
            ...
            try:
                ...
                string_date_p1, _ = string_date.rsplit('-', 1)
                datetime_out = datetime.strptime(string_date_p1, "%Y-%m-%dT%H:%M:%S")
    else:
...
def bool_to_str_bool(a_bool: bool) -> str:
...
def transform_data_type(value, oracle_data_type: str, nullable: str):
    ...
    if oracle_data_type in ("VARCHAR2", "CHAR") and isinstance(value, bool):
        value = bool_to_str_bool(value)
    elif oracle_data_type in ("VARCHAR2", "CHAR", "CLOB"):
...
def target_data_model(owner: str, table_name: str, db) -> pd.DataFrame:
    ...
    """, parameters={"owner": owner, "table_name": table_name})
    df.columns = [
        "col_name_target",
        "data_type_target",
        "data_length_target",
