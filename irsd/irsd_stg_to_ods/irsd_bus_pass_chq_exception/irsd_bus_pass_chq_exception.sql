...
-- build temporary table of current and previous Benefit Months for use in logic below
create table IRSD_BUS_PASS_BEN_MONTH nologging compress as
  select  
    CURR_BEN_MTH_PART_NUM, PREV_BEN_MTH_PART_NUM
    ,CURR_BEN_MTH, CURR_CUTOFF
    ,PREV_BEN_MTH, PREV_CUTOFF
  from (
    select  ROW_NUMBER() over (order by dm_load_sk desc) "ROW_NUM",
      to_char(max(pmt_bnft_mth_dt) over (partition by source_system_cd order by dm_load_sk desc),'YYYYMM') "CURR_BEN_MTH_PART_NUM",
      to_char(lead(pmt_bnft_mth_dt) over (partition by source_system_cd order by dm_load_sk desc),'YYYYMM') "PREV_BEN_MTH_PART_NUM",
      max(pmt_bnft_mth_dt) over (partition by source_system_cd order by dm_load_sk desc) "CURR_BEN_MTH",
      max(extract_asof_dt) over (partition by source_system_cd order by dm_load_sk desc) "CURR_CUTOFF",
      lead(pmt_bnft_mth_dt) over (partition by source_system_cd order by dm_load_sk desc) "PREV_BEN_MTH",
      lead(extract_asof_dt) over (partition by source_system_cd order by dm_load_sk desc) "PREV_CUTOFF",
      last_day(add_months(trunc(sysdate)-1,-1))+1 "CURR_MONTH",
      last_day(add_months(trunc(sysdate)-1,-1))+1 "PREV_MONTH",
      last_day(add_months(trunc(sysdate)-1,-1))+1 "PREV2_MONTH"
    from CDW.MIS_LOAD_CONTROL
    order by dm_load_sk desc
  )
  where ROW_NUM = 1
;
...
create table IRSD_BUS_PASS_O_CHQ_TMP nologging compress parallel as
  select      /*+ parallel */ distinct 
    ...
    <= last_day(m.CURR_BEN_MTH)
    and o_d.X_EFF_END_DT >= m.CURR_BEN_MTH
    ...
    then o_d.X_APPROVAL_DT else o_d.X_EFF_START_DT end) 
    <= last_day(m.CURR_BEN_MTH)
    and o_d.X_EFF_END_DT >= m.CURR_BEN_MTH -- (1a) Current
    ...
    when  (case when o_d.X_APPROVAL_DT > o_d.X_EFF_START_DT 
    then o_d.X_APPROVAL_DT else o_d.X_EFF_START_DT end) 
    > last_day(m.CURR_BEN_MTH)                                    -- (2a) Future dated
    and o_d.STATUS_CD in ('Approved')--,'Pending')
    and length(o_d.x_serial_num) = 20
    and ois.INTEGRATION_STATE = 'Synced'
  ...
  and     bp.BNFTPLAN_END_DT >= m.CURR_BEN_MTH
  and     bp.BNFTPLAN_START_DT <= last_day(m.CURR_BEN_MTH)
  inner join  ICM_STG.W_ORDER_F o_f on c.ROW_WID=o_f.case_wid 
  and     o_f.X_SEC_BU_ID = 'ICMDW-1000' and o_f.DELETE_FLG='N'
  inner join  ICM_STG.W_ORDER_D o_d 
    on o_d.ROW_WID=o_f.ORDER_WID 
      and o_d.DELETE_FLG='N'
    and o_d.X_EFF_END_DT  >= m.CURR_BEN_MTH
    and o_d.X_EFF_START_DT  <= last_day(add_months(m.CURR_BEN_MTH, 1))
    and o_d.STATUS_CD in ('Ready for Approval','Pending','Approved','Cancelled')--,'Archived','Closed')
  left join ICM_STG.W_ORDERITEM_F oi on oi.order_wid=o_f.order_wid and oi.DELETE_FLG='N'
  left join ICM_STG.WC_ORDER_INT_STATE_DM ois on ois.ORDER_WID= o_f.ORDER_WID 
...
create table IRSD_BUS_PASS_MIS_CHQ_TMP nologging compress  parallel as
select  /*+ parallel append */ t.*
  ,case when CD24_PREV_BEN_MTH_CNT + CD87_PREV_BEN_MTH_CNT 
    + CD49_PREV_BEN_MTH_CNT + CD49_PASS_PREV_BEN_MTH_CNT > 0 then 'DA Case Receiving'
  ...
  ,case when CD24_CURR_BEN_MTH_CNT + CD87_CURR_BEN_MTH_CNT 
    + CD49_CURR_BEN_MTH_CNT + CD49_PASS_CURR_BEN_MTH_CNT > 0 then 'DA Case Receiving'
    || case when CD49_PASS_CURR_BEN_MTH_CNT  > 0   then ' TS (pass)'   end
...
from (
  select  MIS_FILE_NUM
    ,nvl(max(CD24_PREV_BEN_MTH_AMT), 0) "CD24_PREV_BEN_MTH_AMT", nvl(max(CD87_PREV_BEN_MTH_AMT), 0) "CD87_PREV_BEN_MTH_AMT", nvl(max(CD49_PREV_BEN_MTH_AMT), 0) "CD49_PREV_BEN_MTH_AMT"
    ,nvl(max(CD24_CURR_BEN_MTH_AMT), 0) "CD24_CURR_BEN_MTH_AMT", nvl(max(CD87_CURR_BEN_MTH_AMT), 0) "CD87_CURR_BEN_MTH_AMT", nvl(max(CD49_CURR_BEN_MTH_AMT), 0) "CD49_CURR_BEN_MTH_AMT"
    ,nvl(max(CD24_PREV_BEN_MTH_CNT), 0) "CD24_PREV_BEN_MTH_CNT", nvl(max(CD87_PREV_BEN_MTH_CNT), 0) "CD87_PREV_BEN_MTH_CNT", nvl(max(DA_PREV_BEN_MTH_CD), 0)  "DA_PREV_BEN_MTH_CD", nvl(max(CD49_PREV_BEN_MTH_CNT), 0) "CD49_PREV_BEN_MTH_CNT", nvl(max(CD49_PASS_PREV_BEN_MTH_CNT), 0) "CD49_PASS_PREV_BEN_MTH_CNT"
    ,nvl(max(CD24_CURR_BEN_MTH_CNT), 0) "CD24_CURR_BEN_MTH_CNT", nvl(max(CD87_CURR_BEN_MTH_CNT), 0) "CD87_CURR_BEN_MTH_CNT", nvl(max(DA_CURR_BEN_MTH_CD), 0)  "DA_CURR_BEN_MTH_CD", nvl(max(CD49_CURR_BEN_MTH_CNT), 0) "CD49_CURR_BEN_MTH_CNT", nvl(max(CD49_PASS_CURR_BEN_MTH_CNT), 0) "CD49_PASS_CURR_BEN_MTH_CNT"
  from
  (
  --  Client opted for Code 49 Payment instead of Bus Pass 
    ...
    --
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '24' then round(item_adj_amt/52,0) end),0)    "CD24_PREV_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '24' then item_adj_amt end),0)                "CD24_PREV_BEN_MTH_AMT"
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '87' then round(item_adj_amt/-52,0) end),0)   "CD87_PREV_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '87' then item_adj_amt end),0)                "CD87_PREV_BEN_MTH_AMT"
              ,max(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and f.CORE_BUS_SK=1 then 'DA' end)                                   "DA_PREV_BEN_MTH_CD"
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '49' then round(item_adj_amt/52,0) end),0)    "CD49_PREV_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.PREV_BEN_MTH_PART_NUM and allowance_cd = '49' then item_adj_amt end),0)                "CD49_PREV_BEN_MTH_AMT"
              ,0                                                                                                                              "CD49_PASS_PREV_BEN_MTH_CNT"
    
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '24' then round(item_adj_amt/52,0) end),0)    "CD24_CURR_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '24' then item_adj_amt end),0)                "CD24_CURR_BEN_MTH_AMT"
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '87' then round(item_adj_amt/-52,0) end),0)   "CD87_CURR_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '87' then item_adj_amt end),0)                "CD87_CURR_BEN_MTH_AMT"
              ,max(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and f.CORE_BUS_SK=1 then 'DA' end)                                   "DA_CURR_BEN_MTH_CD"
              ,nvl(max(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '49' then round(item_adj_amt/52,0) end),0)    "CD49_CURR_BEN_MTH_CNT"
              ,nvl(sum(case when f.ASST_MTH_PART_NUM=m.CURR_BEN_MTH_PART_NUM and allowance_cd = '49' then item_adj_amt end),0)                "CD49_CURR_BEN_MTH_AMT"
              ,0                                                                                                                              "CD49_PASS_CURR_BEN_MTH_CNT"
--      
    from        CDW.FN_PAYMENT_ITEM_F      f
    inner join  IRSD_BUS_PASS_BEN_MONTH    m  on 1=1
    where       f.ASST_MTH_PART_NUM >= least(m.PREV_BEN_MTH_PART_NUM, to_char(last_day(add_months(trunc(sysdate)-1,-3))+1,'yyyymm'))
    ...
    select      /*+ parallel append */ 
                ...
                ,null, null, null, null, case when CAS_BUSPASS_CNT > 0 then 'DA' end, null, null
                ,CAS_BUSPASS_CNT                                                "CD49_PASS_PREV_BEN_MTH_CNT"
                ,null, null, null, null, null, null, null
                ,0                                                              "CD49_PASS_CURR_BEN_MTH_CNT"
    from        mis_stg.TPHIST_FULL_TABLE  t
    inner join  IRSD_BUS_PASS_BEN_MONTH    m  on 1=1
    where       PMT_BNFT_MTH_DT = m.PREV_BEN_MTH
  ...
  --    and     CAS_PROG_ACTV_CD='N'                                              -- include rows that indicate this payment record did NOT go to CAS (introduced 2018-01 for Bus Pass)
    ...
    from        mis_stg.TPHIST_FULL_TABLE  t
    inner join  IRSD_BUS_PASS_BEN_MONTH    m  on 1=1
    where       PMT_BNFT_MTH_DT = m.CURR_BEN_MTH
      and       PMT_BNFT_MTH_DT >= to_date(20180101,'yyyymmdd')                   -- "TS" started on 2018-Jan Benefit Month
      and       PMT_ISS_CD || PMT_PAYEE_CD  in ('AC', 'BC', 'CC')                 -- only Cheque records should be counted. Also, ignore reversals (ask Lyn Munz)
  --    and     CAS_PROG_ACTV_CD='N'                                              -- include rows that indicate this payment record did NOT go to CAS (introduced 2018-01 for Bus Pass)
  )
...
create table IRSD_BUS_PASS_EA_CHQ_TMP nologging compress parallel as
select      /*+ parallel append */ distinct
            CURR_BEN_MTH                                                       "BEN_MTH_DT"
...
--
            ...
            ,case when ea.X_CLASS_CD='08 Medical Only' 
              and EA.X_CLASS_START_DT <= m.CURR_BEN_MTH
...
--
            ...
            ,BUS_PASS_DERIVE_VALUE('Key Player - Bus Pass', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)   "KP_BP_CNT"
            ,BUS_PASS_DERIVE_VALUE('Spouse - Bus Pass', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)       "SP_BP_CNT"
            ,BUS_PASS_DERIVE_VALUE('Key Player - MIS Pass', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)   "KP_MIS_PASS_CNT"
            ,BUS_PASS_DERIVE_VALUE('Spouse - MIS Pass', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)       "SP_MIS_PASS_CNT"
            ,BUS_PASS_DERIVE_VALUE('Key Player - MIS Cash', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)   "KP_MIS_CASH_CNT"
            ,BUS_PASS_DERIVE_VALUE('Spouse - MIS Cash', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)       "SP_MIS_CASH_CNT"
            ,case when sp.ROW_WID is not null then 'Y' else 'N' end                                                                                                                                               "HAS_SPOUSE_FLG"
            ,BUS_PASS_DERIVE_VALUE('Data Exception', bp_kp.VALID_FLG, bp_sp.VALID_FLG, case when sp.ROW_WID is not null then 'Y' end, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT
...
left join   IRSD_BUS_PASS_O_CHQ_TMP bp_kp on bp_kp.CONTACT_WID=kp.ROW_WID
...
left join   IRSD_BUS_PASS_O_CHQ_TMP bp_sp on bp_sp.CONTACT_WID=sp.ROW_WID
left join   IRSD_BUS_PASS_MIS_CHQ_TMP mis on mis.MIS_FILE_NUM=ea.X_LEGACY_FILE_NUM
where       ea.TYPE_CD='Employment and Assistance' 
and         ea.DELETE_FLG='N'
and         case 
              when ea.STATUS_CD = 'Closed' and nvl(trunc(ea.CASE_CLOSED_DT),m.CURR_BEN_MTH) >= m.CURR_BEN_MTH then 'Y' 
...
-- merge BP and EA and MIS
create table IRSD_BUS_PASS_CHQ_EXCEPTION nologging compress  as 
(
    select      /*+ parallel append */ 'EA' "MATCH_TYPE", t.* from IRSD_BUS_PASS_EA_CHQ_TMP t
  ...
  --
              ...
              ,to_char(null)
  --
    from        IRSD_BUS_PASS_O_CHQ_TMP o
    inner join  IRSD_BUS_PASS_BEN_MONTH m on 1=1
    left join   IRSD_BUS_PASS_EA_CHQ_TMP kp on BP_CASE_WID = kp.KP_BP_CASE_WID 
    left join   IRSD_BUS_PASS_EA_CHQ_TMP sp on BP_CASE_WID = sp.SP_BP_CASE_WID 
  ...
  --
              ,null
              ,null
              ,BUS_PASS_DERIVE_VALUE('Key Player - MIS Pass', null, null, null, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)   "KP_MIS_PASS_CNT"
              ,BUS_PASS_DERIVE_VALUE('Spouse - MIS Pass', null, null, null, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)       "SP_MIS_PASS_CNT"
              ,BUS_PASS_DERIVE_VALUE('Key Player - MIS Cash', null, null, null, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)   "KP_MIS_CASH_CNT"
              ,BUS_PASS_DERIVE_VALUE('Spouse - MIS Cash', null, null, null, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)       "SP_MIS_CASH_CNT"
              ,'N'                                                                                                                                   "HAS_SPOUSE_FLG"
              ,BUS_PASS_DERIVE_VALUE('Data Exception', null, null, null, mis.CD49_PASS_CURR_BEN_MTH_CNT, mis.CD49_CURR_BEN_MTH_CNT)          "CDW_EXCEPTION_TXT"
              ,to_char(null)
  --
    from        IRSD_BUS_PASS_MIS_CHQ_TMP mis
    inner join  IRSD_BUS_PASS_BEN_MONTH m on 1=1
    left join   IRSD_BUS_PASS_EA_CHQ_TMP ea on ea.MIS_FILE_NUM = mis.MIS_FILE_NUM
