...
create table IRSD_BUS_PASS_O_TMP nologging compress parallel as
	select /*+ parallel */ distinct 
		...
		max(
			case 
				when (
					case 
						when o_d.X_APPROVAL_DT > o_d.X_EFF_START_DT 
						then o_d.X_APPROVAL_DT 
						else o_d.X_EFF_START_DT 
					end
				...
				then o_d.x_serial_num 
			end
		) over (partition by con.row_wid)      "CANCELLED_SERIAL_NUM"
	...
	inner join ICM_STG.WC_CASE_PER_XM con_xm 
		on c.row_wid=con_xm.case_wid 
			and con_xm.DELETE_FLG='N' 
			and con_xm.END_DATE is null 
			and con_xm.SUBJECT_FLG='Y' 
			and con_xm.RELATIONSHIP_CD='Key player'
	inner join ICM_STG.W_PARTY_PER_D con 
		on con.ROW_WID=con_xm.CONTACT_WID 
			and con.DELETE_FLG='N' 
	inner join  ICM_STG.WC_BNFT_PLAN_CASE_XM bp_xm 
		on bp_xm.case_wid=c.ROW_WID 
			and bp_xm.DELETE_FLG='N' 
	inner join  ICM_STG.W_BNFT_PLAN_D bp 
		on bp_xm.BNFT_PLAN_WID = bp.ROW_WID 
			and bp.DELETE_FLG='N' 
	...
	inner join  ICM_STG.W_ORDER_F o_f 
		on c.ROW_WID=o_f.case_wid 
			and o_f.X_SEC_BU_ID = 'ICMDW-1000' 
			and o_f.DELETE_FLG='N'
	inner join ICM_STG.W_ORDER_D o_d 
		on o_d.ROW_WID=o_f.ORDER_WID 
			and o_d.DELETE_FLG='N'
			and o_d.X_EFF_END_DT  >= trunc(add_months(to_date(substr(m.ASOF_DATE_NAME,1,10),'yyyy-mm-dd'), 0),'mm') -- tss change to current month and test
	...
	left join ICM_STG.W_ORDERITEM_F oi 
		on oi.order_wid = o_f.order_wid 
			and oi.DELETE_FLG ='N'
	left join ICM_STG.WC_ORDER_INT_STATE_DM ois 
		on ois.ORDER_WID = o_f.ORDER_WID 
...
create table IRSD_BUS_PASS_MIS_TMP nologging compress  parallel as
...
create table IRSD_BUS_PASS_EA_TMP nologging compress parallel as
	...
	,mis.CD49_PASS_PREV_BEN_MTH_CNT     "DA_CLIENT_PREV_PASS_CNT"
	,mis.CD49_PREV_BEN_MTH_CNT          "DA_CLIENT_PREV_CASH_CNT"
	,mis.CD49_PREV_BEN_MTH_AMT          "DA_CLIENT_PREV_CASH_AMT"
	,mis.CD49_PASS_CURR_BEN_MTH_CNT     "DA_CLIENT_CURR_PASS_CNT"
	,mis.CD49_CURR_BEN_MTH_CNT          "DA_CLIENT_CURR_CASH_CNT"
	,mis.CD49_CURR_BEN_MTH_AMT          "DA_CLIENT_CURR_CASH_AMT"
	--
	...
	left join   IRSD_BUS_PASS_O_TMP bp_kp on bp_kp.CONTACT_WID=kp.ROW_WID
	...
	left join   IRSD_BUS_PASS_O_TMP bp_sp on bp_sp.CONTACT_WID=sp.ROW_WID
	left join   IRSD_BUS_PASS_MIS_TMP mis on mis.MIS_FILE_NUM=ea.X_LEGACY_FILE_NUM
...
-- delete previous version for this month
-- delete IRSD_BUS_PASS_ARCHIVE where BEN_MTH_DT = trunc(sysdate,'mm');
delete IRSD_BUS_PASS_ARCHIVE where BEN_MTH_DT = (select trunc(to_date(substr(m.ASOF_DATE_NAME,1,10),'yyyy-mm-dd'),'MONTH')  from CDW.CDW_ASOF_DATES m where SOURCE_SYSTEM_ID='iRSD');

commit;

-- merge BP and EA and MIS into Archive for reporting
insert into IRSD_BUS_PASS_ARCHIVE (
	select /*+ parallel append */ 'EA' "MATCH_TYPE", t.* 
	from IRSD_BUS_PASS_EA_TMP t
	...
	select
		...
		to_char(null),
		to_char(null)
		from        IRSD_BUS_PASS_O_TMP o
		inner join  CDW.CDW_ASOF_DATES m on SOURCE_SYSTEM_ID='iRSD'
		left join   IRSD_BUS_PASS_EA_TMP kp on BP_CASE_WID = kp.KP_BP_CASE_WID 
		left join   IRSD_BUS_PASS_EA_TMP sp on BP_CASE_WID = sp.SP_BP_CASE_WID 
		...
		to_char(null)
	from IRSD_BUS_PASS_MIS_TMP mis
	inner join CDW.CDW_ASOF_DATES m on SOURCE_SYSTEM_ID='iRSD'
	left join IRSD_BUS_PASS_EA_TMP ea on ea.MIS_FILE_NUM = mis.MIS_FILE_NUM
...
create index IRSD_BUS_PASS_ARCHIVE_KP 
on IRSD_BUS_PASS_ARCHIVE(BEN_MTH_DT, KP_CONTACT_WID) 
local parallel 
tablespace ODS_INDX
;

create index IRSD_BUS_PASS_ARCHIVE_SP 
on IRSD_BUS_PASS_ARCHIVE(BEN_MTH_DT, SP_CONTACT_WID) 
local parallel 
tablespace ODS_INDX
;
